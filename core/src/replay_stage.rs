...
impl ReplayStage {
    pub fn new(
        ...
        let run_replay = move || {
            ...
            let mut last_logged_vote_slot = u64::MAX;
            ...
            loop {
                ...
                for slot in newly_computed_slot_stats {
                    ...
                    let (duplicate_confirmed_forks, mostly_confirmed_slots) =
                ...
                if let Some((ref vote_bank, ref switch_fork_decision)) = vote_bank {
                        ...
                        ReplayStage::first_mostly_confirmed_bank(vote_bank.clone(), &progress)
                    {
                        ...
                        vote_banks.extend(
                            ...
                            .into_iter()
                            .take(tower.vo_config.threshold_ahead_count + 1)
                        ...
                        let after_skip_threshold = tower.vo_config.after_skip_threshold;
                        ...
                        if vote_banks.is_empty() {
                            let threshold_escape_count = tower.vo_config.threshold_escape_count;
    ...
    fn handle_votable_banks(
        ...
        if banks.is_empty() {
            warn!("handle votable banks: empty banks");
            return Ok(());
        }
    ...
    fn mark_slots_duplicate_confirmed(
        ...
        for slot in mostly_confirmed_slots.iter() {
            progress
                .get_fork_stats_mut(*slot)
                .map(|fs| fs.is_mostly_confirmed = true);
        }
    }
